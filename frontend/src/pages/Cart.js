...
const Cart = () => {
  ...
  const handleWhatsAppOrder = () => {
    ...
    const productList = data.map(item => (
      `🖼️ ${item.productId.productImage[0]}\n*${item.productId.productName}* x${item.quantity} = ₹${item.productId.sellingPrice * item.quantity}`
    )).join('\n\n');
      ...
      <div className="flex flex-col lg:flex-row gap-8 mt-4">
        <div className="w-full lg:w-2/3 space-y-4">
          {data.length === 0 && !loading ? (
            ...
            data.map((product) => (
              <div key={product._id} className="bg-white p-4 rounded border grid grid-cols-[100px_1fr] gap-4">
                <img src={product.productId.productImage[0]} className="object-contain h-24 w-24 rounded" alt="Product" />
                <div className="relative">
                  <button
                    className="absolute right-0 text-red-600 hover:text-white hover:bg-red-600 p-2 rounded-full"
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <button
                      className="border border-red-600 text-red-600 w-6 h-6 rounded hover:bg-red-600 hover:text-white"
