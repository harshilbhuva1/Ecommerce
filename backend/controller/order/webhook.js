...
const webhook = async (req, res) => {
  ...
  if (event === "payment.captured" && payment) {
    ...
    let address = {};
    try {
      address = payment.notes.address ? JSON.parse(payment.notes.address) : {};
    } catch (err) {
    ...
    try {
      ...
      const productDetails = cartItems.map(item => ({
        ...
        price: item.price,
        quantity: item.quantity,
        image: item.image || item.productId?.productImage?.[0] || null
      }));
      ...
      if (saved?._id) {
        ...
        await sendMail({
          ...
          html: `
            ...
            <p><b>Total Amount:</b> ₹${orderDetails.totalAmount}</p>
            <p><b>Order Date:</b> ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}</p>
          `
        });
  ...
  // For other events or if payment entity is missing
  return res.status(200).json({ success: true });
};
